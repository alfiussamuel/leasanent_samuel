...
class LeaSalesTargetStore(models.Model):
    _name = 'lea.sales.target.store'
    _rec_name = 'year'
    _order = 'year desc'
    ...
    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetStore, self).write(vals)
    ...
    channel_ids     = fields.One2many(comodel_name='lea.sales.target.store.channel',inverse_name='reference',string='Channel', domain=[('area','=','all')])
    brt_ids     = fields.One2many(comodel_name='lea.sales.target.store.channel',inverse_name='reference',string='BRT', domain=[('area','=','brt')])
    tmr_ids     = fields.One2many(comodel_name='lea.sales.target.store.channel',inverse_name='reference',string='TMR', domain=[('area','=','tmr')])
    mdn_ids     = fields.One2many(comodel_name='lea.sales.target.store.channel',inverse_name='reference',string='MDN', domain=[('area','=','mdn')])
    ...
    qty_channel_ids     = fields.One2many(comodel_name='lea.sales.target.qty.store.channel',inverse_name='reference',string='Channel', domain=[('area','=','all')])
    qty_brt_ids     = fields.One2many(comodel_name='lea.sales.target.qty.store.channel',inverse_name='reference',string='BRT', domain=[('area','=','brt')])
    qty_tmr_ids     = fields.One2many(comodel_name='lea.sales.target.qty.store.channel',inverse_name='reference',string='TMR', domain=[('area','=','tmr')])
    qty_mdn_ids     = fields.One2many(comodel_name='lea.sales.target.qty.store.channel',inverse_name='reference',string='MDN', domain=[('area','=','mdn')])
    ...
    def button_generate_value(self):
        if self.total_target > 0:
            ...
            if self.area == 'all':
                ...
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LS')])
                for c in channel_ids:
                    self.env['lea.sales.target.store.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
                ...
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LS'),('wh_area_id.name','=','BRT')])
                for c in channel_ids:
                    self.env['lea.sales.target.store.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
                ...
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LS'),('wh_area_id.name','=','TMR')])
                for c in channel_ids:
                    self.env['lea.sales.target.store.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
                ...
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LS'),('wh_area_id.name','=','MDN')])
                for c in channel_ids:
                    self.env['lea.sales.target.store.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
    ...
    def button_generate_qty(self):
        if self.qty_total_target > 0:
            ...
            if self.qty_area == 'all':
                ...
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LS')])
                for c in channel_ids:
                    self.env['lea.sales.target.qty.store.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
                       'contribution'   : 100.00 / float(len(channel_ids)),
                       'ratio_month1'   : self.qty_default_month1,
...
#TARGET BY VALUE
class LeaSalesTargetStoreChannel(models.Model):
    _name = 'lea.sales.target.store.channel'
    _rec_name = 'store_id'
    ...
    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetStoreChannel, self).write(vals)
        self.lock_default_target_ratio()
        return res

    reference               = fields.Many2one(comodel_name='lea.sales.target.store',string='Target ID', ondelete="cascade" )
    ...
    category_ids            = fields.One2many(comodel_name='lea.sales.target.store.channel.category',inverse_name='reference',string='Category')
    area = fields.Selection([('all','All'),('brt','BRT'),('tmr','TMR'),('mdn','MDN')], "Area")


class LeaSalesTargetStoreChannelCategory(models.Model):
    _name = 'lea.sales.target.store.channel.category'
    ...
    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetStoreChannelCategory, self).write(vals)
        self.lock_default_target_ratio()
        return res

    reference               = fields.Many2one(comodel_name='lea.sales.target.store.channel',string='Channel ID', ondelete="cascade" )
...
#TARGET BY QTY
class LeaSalesTargetQtyStoreChannel(models.Model):
    _name = 'lea.sales.target.qty.store.channel'
    _rec_name = 'store_id'
    ...
    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetQtyStoreChannel, self).write(vals)
        self.lock_default_target_ratio()
        return res
    ...
    ratio_month11           = fields.Float('November', default=100)
    ratio_month12           = fields.Float('December', default=100)
    category_ids            = fields.One2many(comodel_name='lea.sales.target.qty.store.channel.category',inverse_name='reference',string='Category')
    area = fields.Selection([('all','All'),('brt','BRT'),('tmr','TMR'),('mdn','MDN')], "Area")


class LeaSalesTargetQtyStoreChannelCategory(models.Model):
    _name = 'lea.sales.target.qty.store.channel.category'
    _rec_name = 'category_id'
    ...
    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetQtyStoreChannelCategory, self).write(vals)
        self.lock_default_target_ratio()
        return res

    reference               = fields.Many2one(comodel_name='lea.sales.target.qty.store.channel',string='Channel ID', ondelete="cascade" )
    category_id             = fields.Many2one(comodel_name='lea.product.moving.status',string='Category')
    contribution            = fields.Float('Contribution (%)')
    contribution_value      = fields.Float('Contribution (Rp.)', compute='compute_contribution_value', store=True)
    ratio_month1            = fields.Float('January', default=100)
