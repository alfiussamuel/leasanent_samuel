...
class LeaSalesTargetConsignment(models.Model):
    _name = 'lea.sales.target.consignment'
    _rec_name = 'year'
    _order = 'year desc'
    ...
    def lock_contribution_channel(self):
        if self.channel_ids :
            ...
            for rec in self.channel_ids:
                ...
                if float(contribution2) <= 99.99 or float(contribution2) >= 100.01:
                    raise Warning('Pembagian contribution category belum sesuai, harap sesuaikan jumlah menjadi 100% ! ' + 'di ' + rec.store_id.name)
        ...
        if self.qty_channel_ids :
            ...
            for rec in self.qty_channel_ids:
                ...
                print 'all_contribution_qty-====================',contribution_qty
                for x in rec.category_ids :
                    contribution_qty2 += x.contribution
                if float(contribution_qty2) <= 99.99 or float(contribution_qty2) >= 100.01:
                    raise Warning('Pembagian contribution category qty belum sesuai, harap sesuaikan jumlah menjadi 100% ! ' + 'di ' + rec.store_id.name)
        ...
        if self.qty_brt_ids :
            ...
            for rec in self.qty_brt_ids:
                contribution_qty2 = 0
                contribution_qty += rec.contribution
                print 'brt_contribution_qty-====================',contribution_qty
                for x in rec.category_ids :
                    contribution_qty2 += x.contribution
                if float(contribution_qty2) <= 99.99 or float(contribution_qty2) >= 100.01:
                    raise Warning('Pembagian contribution category qty belum sesuai, harap sesuaikan jumlah menjadi 100% ! ' + 'di ' + rec.store_id.name)
        ...
        if self.qty_tmr_ids :
            ...
            for rec in self.qty_tmr_ids:
                contribution_qty2 = 0
                contribution_qty += rec.contribution
                print 'tmr_contribution_qty-====================',contribution_qty
                for x in rec.category_ids :
                    contribution_qty2 += x.contribution
                if float(contribution_qty2) <= 99.99 or float(contribution_qty2) >= 100.01:
                    raise Warning('Pembagian contribution category qty belum sesuai, harap sesuaikan jumlah menjadi 100% ! ' + 'di ' + rec.store_id.name)
        ...
        if self.qty_mdn_ids :
            ...
            for rec in self.qty_mdn_ids:
                contribution_qty2 = 0
                contribution_qty += rec.contribution
                print 'mdn_contribution_qty-====================',contribution_qty
                for x in rec.category_ids :
                    contribution_qty2 += x.contribution
                if float(contribution_qty2) <= 99.99 or float(contribution_qty2) >= 100.01:
                    raise Warning('Pembagian contribution category qty belum sesuai, harap sesuaikan jumlah menjadi 100% ! ' + 'di ' + rec.store_id.name)
    ...
    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetConsignment, self).write(vals)
    ...
    consignment_area = fields.Selection([('all','All'),('area','Area')], "Pilih Area", default="all")
    consignment_area_id = fields.Many2one("lea.area", "Area")
    consignment_area_id_name = fields.Char("Area Name", related='consignment_area_id.name')
    consignment_qty_area = fields.Selection([('all','All'),('area','Area')], "Pilih Area", default="all")
    consignment_qty_area_id = fields.Many2one("lea.area", "Area")
    consignment_qty_area_id_name = fields.Char("Qty Area Name", related='consignment_qty_area_id.name')

    #BY VALUE
    total_target    = fields.Float('Total Target')
    avg_target      = fields.Float('Avg. Target/Month', compute='compute_avg_target', store=True)
    channel_ids     = fields.One2many(comodel_name='lea.sales.target.consignment.channel',inverse_name='reference',string='Channel', domain=[('area','=','all')])
    brt_ids     = fields.One2many(comodel_name='lea.sales.target.consignment.channel',inverse_name='reference',string='BRT', domain=[('area','=','brt')])
    tmr_ids     = fields.One2many(comodel_name='lea.sales.target.consignment.channel',inverse_name='reference',string='TMR', domain=[('area','=','tmr')])
    mdn_ids     = fields.One2many(comodel_name='lea.sales.target.consignment.channel',inverse_name='reference',string='MDN', domain=[('area','=','mdn')])
    ...
    qty_channel_ids     = fields.One2many(comodel_name='lea.sales.target.qty.consignment.channel',inverse_name='reference',string='Channel', domain=[('area','=','all')])
    qty_brt_ids     = fields.One2many(comodel_name='lea.sales.target.qty.consignment.channel',inverse_name='reference',string='BRT', domain=[('area','=','brt')])
    qty_tmr_ids     = fields.One2many(comodel_name='lea.sales.target.qty.consignment.channel',inverse_name='reference',string='TMR', domain=[('area','=','tmr')])
    qty_mdn_ids     = fields.One2many(comodel_name='lea.sales.target.qty.consignment.channel',inverse_name='reference',string='MDN', domain=[('area','=','mdn')])
    ...
    def button_generate_value(self):
        if self.total_target > 0:
            ...
            if self.consignment_area == 'all':
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LC')])
                for c in channel_ids:
                    self.env['lea.sales.target.consignment.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
            ...
            elif self.consignment_area == 'area' and self.consignment_area_id.name == 'BRT':
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LC'),('wh_area_id.name','=','BRT')])
                for c in channel_ids:
                    self.env['lea.sales.target.consignment.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
            ...
            elif self.consignment_area == 'area' and self.consignment_area_id.name == 'TMR':
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LC'),('wh_area_id.name','=','TMR')])
                for c in channel_ids:
                    self.env['lea.sales.target.consignment.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
            ...
            elif self.consignment_area == 'area' and self.consignment_area_id.name == 'MDN':
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LC'),('wh_area_id.name','=','MDN')])
                for c in channel_ids:
                    self.env['lea.sales.target.consignment.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
    ...
    def button_generate_qty(self):
        if self.qty_total_target > 0:
            ...
            if self.consignment_qty_area == 'all':
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LC')])
                for c in channel_ids:
                    self.env['lea.sales.target.qty.consignment.channel'].sudo().create({
                       'reference'      : self.id,
                       'store_id'       : c.id,
                       'contribution'   : 100.00 / float(len(channel_ids)),
                       'ratio_month1'   : self.qty_default_month1,
            ...
            elif self.consignment_qty_area == 'area' and self.consignment_qty_area_id.name == 'BRT':
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LC'),('wh_area_id.name','=','BRT')])
                for c in channel_ids:
                    self.env['lea.sales.target.qty.consignment.channel'].sudo().create({
            ...
            elif self.consignment_qty_area == 'area' and self.consignment_qty_area_id.name == 'TMR':
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LC'),('wh_area_id.name','=','TMR')])
                for c in channel_ids:
                    self.env['lea.sales.target.qty.consignment.channel'].sudo().create({
            ...
            elif self.consignment_qty_area == 'area' and self.consignment_qty_area_id.name == 'MDN':
                # channel_ids = self.env['stock.warehouse'].search([('code','=like','LS_%')])
                channel_ids = self.env['stock.warehouse'].search([('wh_type','=','LC'),('wh_area_id.name','=','MDN')])
                for c in channel_ids:
                    self.env['lea.sales.target.qty.consignment.channel'].sudo().create({
...
#TARGET BY VALUE
class LeaSalesTargetConsignmentChannel(models.Model):
    _name = 'lea.sales.target.consignment.channel'
    _rec_name = 'store_id'
    ...
    def lock_default_target_ratio(self):
        if (self.ratio_month1+self.ratio_month2+self.ratio_month3+self.ratio_month4+self.ratio_month5+self.ratio_month6+self.ratio_month7+self.ratio_month8+self.ratio_month9+self.ratio_month10+self.ratio_month11+self.ratio_month12) != 1200:
            raise Warning('Pembagian default ratio bulanan belum sesuai, harap sesuaikan jumlah menjadi 1.200% ! ' + 'di ' + self.store_id.name)

    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetConsignmentChannel, self).write(vals)
        self.lock_default_target_ratio()
        return res


    reference               = fields.Many2one(comodel_name='lea.sales.target.consignment',string='Target ID', ondelete="cascade" )
    ...
    category_ids            = fields.One2many(comodel_name='lea.sales.target.consignment.channel.category',inverse_name='reference',string='Category')
    area = fields.Selection([('all','All'),('brt','BRT'),('tmr','TMR'),('mdn','MDN')], "Area")


class LeaSalesTargetConsignmentChannelCategory(models.Model):
    _name = 'lea.sales.target.consignment.channel.category'
    ...
    def lock_default_target_ratio(self):
        if (self.ratio_month1+self.ratio_month2+self.ratio_month3+self.ratio_month4+self.ratio_month5+self.ratio_month6+self.ratio_month7+self.ratio_month8+self.ratio_month9+self.ratio_month10+self.ratio_month11+self.ratio_month12) != 1200:
            raise Warning('Pembagian default ratio bulanan belum sesuai, harap sesuaikan jumlah menjadi 1.200% ! ' + 'di ' + self.reference.store_id.name + ' ' + 'Category ' + self.category_id.name)

    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetConsignmentChannelCategory, self).write(vals)
        self.lock_default_target_ratio()
        return res

    reference               = fields.Many2one(comodel_name='lea.sales.target.consignment.channel',string='Channel ID', ondelete="cascade" )
...
#TARGET BY QTY
class LeaSalesTargetQtyConsignmentChannel(models.Model):
    _name = 'lea.sales.target.qty.consignment.channel'
    _rec_name = 'store_id'
    ...
    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetQtyConsignmentChannel, self).write(vals)
        self.lock_default_target_ratio()
        return res
    ...
    ratio_month11           = fields.Float('November', default=100)
    ratio_month12           = fields.Float('December', default=100)
    category_ids            = fields.One2many(comodel_name='lea.sales.target.qty.consignment.channel.category',inverse_name='reference',string='Category')
    area = fields.Selection([('all','All'),('brt','BRT'),('tmr','TMR'),('mdn','MDN')], "Area")


class LeaSalesTargetQtyConsignmentChannelCategory(models.Model):
    _name = 'lea.sales.target.qty.consignment.channel.category'
    _rec_name = 'category_id'
    ...
    @api.multi
    def write(self, vals):
        res = super(LeaSalesTargetQtyConsignmentChannelCategory, self).write(vals)
        self.lock_default_target_ratio()
        return res

    reference               = fields.Many2one(comodel_name='lea.sales.target.qty.consignment.channel',string='Channel ID', ondelete="cascade" )
    category_id             = fields.Many2one(comodel_name='lea.product.moving.status',string='Category')
    contribution            = fields.Float('Contribution (%)')
    contribution_value      = fields.Float('Contribution (Rp.)', compute='compute_contribution_value', store=True)
    ratio_month1            = fields.Float('January', default=100)
