...
class wizard_sales_details(models.TransientModel):
    ...
    @api.multi
    def get_timezone_datetime(self):
        if self.env.user.tz:
        ...
        sign = str(c_time)[-6][:1]
        sdate = self.start_date + " 00:00:00"
        edate = self.end_date + " 23:59:59"
        if sign == '-':
            sdate = (datetime.strptime(sdate, '%Y-%m-%d %H:%M:%S') + timedelta(hours=hour_tz, minutes=min_tz)).strftime("%Y-%m-%d %H:%M:%S")
            edate = (datetime.strptime(edate, '%Y-%m-%d %H:%M:%S') + timedelta(hours=hour_tz, minutes=min_tz)).strftime("%Y-%m-%d %H:%M:%S")
        if sign == '+':
            sdate = (datetime.strptime(sdate, '%Y-%m-%d %H:%M:%S') - timedelta(hours=hour_tz, minutes=min_tz)).strftime("%Y-%m-%d %H:%M:%S")
            edate = (datetime.strptime(edate, '%Y-%m-%d %H:%M:%S') - timedelta(hours=hour_tz, minutes=min_tz)).strftime("%Y-%m-%d %H:%M:%S")
        return [sdate, edate]
    ...
    def get_total_sales(self, user_lst=None):
        if self:
            ...
            sdate, edate = self.get_timezone_datetime()
            pos_ids = pos_obj.search([('date_order', '>=', sdate), \
                                      ('date_order', '<=', edate), \
    ...
    def get_total_returns(self, user_lst=None):
        if self:
            ...
            sdate, edate = self.get_timezone_datetime()
            for pos in pos_order_obj.search([('date_order', '>=', sdate),
                                             ('date_order', '<=', edate), 
                                             ('user_id', 'in', user_ids), ('state', 'in', ['done', 'paid', 'invoiced']),
                                            ('company_id', '=', company_id)]):
    ...
    def get_tax_amount(self, user_lst=None):
        if self:
            ...
            sdate, edate = self.get_timezone_datetime()
            pos_ids = pos_order_obj.search([('date_order','>=', sdate),
                                            ('date_order','<=', edate),
