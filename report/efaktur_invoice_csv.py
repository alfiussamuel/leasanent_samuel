        ...
        for inv in objects:
            ...
            for line in inv.invoice_line_ids:
                ...
                if inv.partner_id.type_counting_margin in ['ds1p','ds2p']:
                    discount = int((line.lea_share_discount + line.lea_margin) / line.subtotal_invoice * 100.00)
                elif inv.partner_id.type_counting_margin == 'tpp':
                    discount = int(line.price_discount / line.subtotal_invoice * 100.00)
                ...
                c_specs = [
                    ...
                    ('price_unit',  1, 0, 'number', int(line.lea_sell_price)),
                    ...
                    ('price_unit_quantity',  1, 0, 'number', int(line.subtotal_invoice)),
                    ('discount',  1, 0, 'number', discount),
                    # ('dpp_product',  1, 0, 'number', int(_p.amount_currency_line(line.price_subtotal, line))),
                    ('dpp_product',  1, 0, 'number', int(line.lea_net_amount)),
                    ('ppn_product',  1, 0, 'number', int(line.lea_net_amount*0.1)),
