odoo.define('pos_promotional_scheme.widgets', function (require){
"use strict";

	console.log("In widgets.js");
	
});

/*function pos_promotional_scheme_widgets(instance, module){ //module is instance.point_of_sale
    	...
    	var module=instance.point_of_sale;
    	   ...
    	   module.PaypadWidget = module.PaypadWidget.extend({
    		    ...
    		    renderElement: function() {
    		        ...
    		        var pos_obj =  this.pos;
    		        $(".scheme-button-click").click(function () {
    		    		self.apply_scheme(pos_obj);
    		    	});
    		    },  	
    	
    		    
    		    apply_scheme : function(pos_obj)
    		    {
	    	    	var orderlines = new Array();
	    	    	var orderlines_unit_price= new Array();
	    	    	pos_obj.get('selectedOrder').get('orderLines').each(function(orderline){
	    	    	orderlines.push(orderline);
	    	    	orderlines_unit_price.push(orderline)
	    	        });
	    	    	var schemes = pos_obj.get('scheme');//Getting all available schemes
	    	    	
	    	    	
	    	    	var available_on = pos_obj.get('available_on_list');//Getting records from loyalty.available_on
	    	    	
	    	    	var get_product_orderlines =  new module.OrderlineCollection();
	    	    	
	    	    	var modified_orderlines_new = new module.OrderlineCollection();
	    	    	var modified_orderlines_new1 = new module.OrderlineCollection();
	    	    	var modified_orderlines_new2 = new module.OrderlineCollection();
	    	    	var neworderLines_new = new module.OrderlineCollection();
	    	    	
	    	    	var buy_orderlines = new module.OrderlineCollection();
	    	    	var get_orderlines = new module.OrderlineCollection();
	    	    	var qty;
	    	    	var new_q = new Array();
	    	    	var get_product_id_list = new Array();
	    	    	var scheme_discount ;
	    	    	//Scheme start
	    	    	var customer = pos_obj.get('customerlists');
	    	        console.log("Schemes: "+schemes['name']);
	    	        console.log("Available on: "+available_on);
	    	        ...
	    	        for(var s=0;s<schemes.length;s++)
	        		{
		        		...
		        		for (loc in schemes[s].locations)
		        		...
		        		if(schemes[s].scheme_type == 'buy_x_get_y')
		        			{
		            			...
		            			{
		            				var neworderLines = new module.OrderlineCollection();
		            				var modified_orderlines = new module.OrderlineCollection();
		            									...
		            									if(prod_list[p] == orderlines[o].get_product().id)
		            										{
		                    									...
		                    									pos_obj.get('selectedOrder').removeOrderline(orderlines[o]);
		            									...
		            									if(getqty >= schemes[s].get_a_qty)
		            										{
		            											...
		            											for(data in temp_orderline_data_new)
		            											{
//		            												console.log(temp_orderline_data_new[data].key+"---"+temp_orderline_data_new[data].value)
		            											}
		            											
		            											var getlines = new Array();
		            											for(item in temp_orderline_data_new)
		            											{
		            												...
		            												if(getlines[v].value >=qty)
		            													{
		        															...
		        															pos_obj.get('selectedOrder').addProduct(line_product,{quantity:qty,discount:schemes[s].discount});
		        	    													var sq = (getlines[v].value-qty)
		        															if(sq > 0)
		        															{
		        																//adding remained product with ful price
		        																var line_product =this.pos.db.get_product_by_id(getlines[v].key);
		        																pos_obj.get('selectedOrder').addProduct(line_product,{quantity:sq});
		        															}
		            													}
		            												else if(getlines[v].value < qty)
		            													{
		        	    													var line_product = this.pos.db.get_product_by_id(getlines[v].key);
		        															pos_obj.get('selectedOrder').addProduct(line_product,{quantity:getlines[v].value,discount:schemes[s].discount});
		            									...
		            									else
		            										{
		            											...
		            											neworderLines.each(function(orderline){
		            												...
		            												pos_obj.get('selectedOrder').addProduct(line_product,{quantity:orderline.quantity});
		                							            });
		                			...
		                			for(var n=0;n<avail.length;n++)
		                				{
		                					...
		                					if(bqty >= schemes[s].buy_a_qty)
		            						{
		                						...
		                						for(item = 0; item<original_orderline_array.length ; item++)
		                							{
//		                								console.log("original_orderline_array==>>"+original_orderline_array[item].new_product_id+"--"+original_orderline_array[item].id+"---"+original_orderline_array[item].price+"---")
		                							}
		            							
		                						original_orderline_array.sort(function(a,b) { return (b.price) - (a.price) } )

		                						for(item = 0; item<original_orderline_array.length ; item++)
		                							{
//		                								console.log("after sorting==>>original_orderline_array==>>"+original_orderline_array[item].id+"---"+original_orderline_array[item].price+"---")
		                							}
		                						var summed_bqty = 0;
		                						var summed_gqty = 0;
		                						var cnt=0;
		                						if(original_orderline_array.length > 0)
		                							...
		                							for(product = 0 ; product < original_orderline_array.length ; product++)
		                    							{
		        										...
		        										pos_obj.get('selectedOrder').addProduct(line_buy_product,{quantity:1,discount:0});
		                    							...
		                    							if(summed_bqty == scheme_bqty)
		                    								...
		                    								for(i=(original_orderline_array.length-1);i>=cnt ;i--)
		                										...
		                										pos_obj.get('selectedOrder').addProduct(line_get_product,{quantity:1,discount:schemes[s].discount,merge:false});
		    	        						...
		    	        						{
		    	        							pos_obj.get('selectedOrder').get('orderLines').add(norderlines2[i]);
		    	        						}
		                						}
   ...
   ////////////////////////////////////////////////////////////////////////////////		        		
		         		...
		         		else if(schemes[s].scheme_type == 'unit_price_disc_amt')
		    			{
		    				...
		    				var scheme_unit_orderline = new module.OrderlineCollection();
		    				var scheme_unit_orderline_less = new module.OrderlineCollection();
		    				var scheme_unit_orderline_more = new module.OrderlineCollection();
		    				var scheme_unit_orderline_remained = new module.OrderlineCollection();
		    				var neworderLines_unique_product = new module.OrderlineCollection();
		    				...
		    				for (x in orderline_data)
		    					...
		    					{
//		    						console.log(temp_orderline_data[data].key+"---"+temp_orderline_data[data].value)
		    					}
		    				...
		    				for(prod in scheme_prod_list)
		    					{
//		    					console.log(scheme_prod_list[prod].key+"---"+scheme_prod_list[prod].value)
		    					}
		    				
		    				var scheme_qty = schemes[s].qty_disc;
		    				var qty_arranged_dict = new Array()
		    				var qty = []
		    				var qty_on = this.pos.get('qty_disc');//Getting records from loyalty.available_on
		    				...
		    				qty_arranged_dict.sort(function(a,b) { return (b.value) - (a.value) } )
		    				...
		    				for(var product=0;product<scheme_prod_list.length;product++)
		    				{
		    							...
		    							if(scheme_prod_list[product].value >= qty_arranged_dict[item].value)
		    								{
		    									...
		    									var prod_obj = this.pos.get('product_obj')
		    									...
		    									pos_obj.get('selectedOrder').addProduct(line_product,{price:scheme_qty_discount,quantity:qty_obtained});
		    									...
		    									while(remained >= least_qty)
		    									{
		    										for(dict_item in qty_arranged_dict)
		    										{
		    											...
		    											{
		    												var line_product =this.pos.db.get_product_by_id(scheme_prod_list[product].key);
		    												pos_obj.get('selectedOrder').addProduct(line_product,{price:sch_qty_disc,quantity:qty_arranged_dict[dict_item].value});
		    									...
		    									if(remained > 0)
		    										{
		    											var line_product = this.pos.db.get_product_by_id(scheme_prod_list[product].key);
		    											pos_obj.get('selectedOrder').addProduct(line_product,{price:product_price,quantity:remained});
		    										}
		    						...
		    						if(product_discounted == false)
		    							{
		    							var line_product =this.pos.db.get_product_by_id(scheme_prod_list[product].key);
		    							pos_obj.get('selectedOrder').addProduct(line_product,{quantity:scheme_prod_list[product].value});
		    							}
		    							...
		    							if(scheme_prod_list[product].value >= qty_arranged_dict[item].value)
		    								{
		    									...
		    									var line_product =this.pos.db.get_product_by_id(scheme_prod_list[product].key);
		    									pos_obj.get('selectedOrder').addProduct(line_product,{discount:scheme_qty_discount,quantity:qty_obtained});
		    									...
		    									while(remained >= least_qty)
		    									{
		    										for(dict_item in qty_arranged_dict)
		    										{
		    											...
		    											{
		    												var line_product =this.pos.db.get_product_by_id(scheme_prod_list[product].key);
		    												pos_obj.get('selectedOrder').addProduct(line_product,{discount:sch_qty_disc,quantity:qty_arranged_dict[dict_item].value});
		    												remained -= qty_arranged_dict[dict_item].value
		    											}
		    										}
		    									}
		   ...
		   ////////////////////////////////////////////////////////////////////////////		        		
		        		...
		        		{
		        		var orderline_data_new = []
		        		var volume_neworderLines = new module.OrderlineCollection();
		        		...
		        		temp_orderline_data_new.sort(function(a,b) { return (b.new_price) - (a.new_price) } )
		        		
		        		console.log("array after sorting")
		        		for(prod in temp_orderline_data_new)
		        			{
//		        				console.log(temp_orderline_data_new[prod].new_id+"--"+temp_orderline_data_new[prod].new_qty+"--"+temp_orderline_data_new[prod].new_price)
		        			}
		    			...
		    			j=1;
		        		for(product in temp_orderline_data_new)
		        			{
		        			for(i=0;i<temp_orderline_data_new[product].new_qty;i++)
		        				{
		        		...
		        		console.log(volume_singleton_array);
		    			for(item = 0; item<volume_singleton_array.length ; item++)
		    				{
//		    					console.log("volume_singleton_array==>>"+volume_singleton_array[item].volume_product_id+"--"+volume_singleton_array[item].volume_id+"---"+volume_singleton_array[item].volume_price+"---")
		    				}
		        		...
		        		if(volume_bqty >= schemes[s].buy_a_qty_in_volume)
		        		{
		        			...
		        			for(prd = 0; prd < volume_singleton_array.length ; prd++)
		        			{
		        				...
		        				{
		        					for(p=0;p<schemes[s].buy_a_qty_in_volume;p++)
		            				{
		        	    				sum_qty = 0;
		        	    				total_price = 0;
		        	    				price_difference = 0;
		        			    		...
		        			    		if(stock_flag == true)
		        			    		{
		        				    		...
		        				    		pos_obj.get('selectedOrder').addProduct(line_product,{discount:disc,merge:true,merge_flag:true,stock:true});
		        			    			stock_flag = false;
		        			    		}
		        			    		else
		        			    			{
		        			    			var line_product =pos_obj.db.get_product_by_id(remained_product_ids[p]);
		        				    		pos_obj.get('selectedOrder').addProduct(line_product,{discount:disc,merge:true,merge_flag:true});
		        			    			}
		        			...
		        			if(remained_product_ids.length !=0)
		        				{
		    	    				for(i=0;i<remained_product_ids.length;i++)
		    	    				{
		    		    	    		...
		    		    	    		pos_obj.get('selectedOrder').addProduct(line_product,{quantity:1});
		    	    				}
		        				}
		        			qty_counter = 0;
		    				price_counter = 0;	
		        		...
		        		else//NOT ENOUGH QTY
		    	    	{
		    	    		...
		    	    		var line_product =pos_obj.db.get_product_by_id(orderline.get_product().id);
		    	    		pos_obj.get('selectedOrder').addProduct(line_product,{price:orderline.price,quantity:orderline.quantity});
		    	    		});
		    	    	}
	    	    	...
	    	    	modified_orderlines.each(function(orderline){
	    	    		var line_product =pos_obj.db.get_product_by_id(orderline.get_product().id);
	    				pos_obj.get('selectedOrder').addProduct(line_product,{quantity:orderline.quantity,discount:orderline.discount});
	    	        });
	    	    	}
	    	    	if(new_q.length!=0)
	    	    		{
	    	    			for(var i=0;i<get_product_id_list.length;i++)
	    	    			{
	    						...
	    						pos_obj.get('selectedOrder').addProduct(line_product,{quantity:new_q[0],discount:scheme_discount});
	    	    			}
	    	    		} 
    		    ...
    		    get_product_details : function(id) {
    		    	...
    		    	var product_list = this.pos.get('product_obj');
    			...
    			get_available_on_product_list : function(id) {
    				var available_on = this.pos.get('available_on_list');//Getting records from loyalty.available_on
    			...
    			get_product_price : function(id) {
    				...
    				var product_obj = this.pos.get('prod_temp');//Getting records from loyalty.available_on
